 ...
 * Acl Class
 * 
 * Acl checks whether user has permission
 * 
 * PHP 5.0 / Laravel 4.0
 * 
...
class Acl {
    ...
    /**
     * Initialize user to check user's permission
     *
    ...
    /**
     * Set curent logged in user or guest user
     */
    ...
    public function isSuperuser() {
        ...
        $superusers = Config::get('acl::superusers');
        if (Auth::user()) {
            $user = Auth::user();
            if ($user->is_admin == 1) {
                $superusers[] = $user->id;
        ...
        return ($this->userId !== null && in_array($this->userId, $superusers));
    }
